...
public class FilmDbStorage implements FilmStorage {
    ...
    public List<Integer> commonFilms(final Integer userId, final Integer otherUserId) {
        String sqlRequst =
                " (SELECT DISTINCT t1.film_id FROM "
                        + " (SELECT film_id  FROM film_likes WHERE user_id = ?) AS t1 "
                        + " INNER JOIN "
                        + " (SELECT film_id  FROM film_likes WHERE user_id = ?) AS t2 "
    ...
    public List<Integer> differentFilms(final Integer mainUserId, final Integer otherUserId) {
        String sqlRequst =
                " SELECT film_id FROM film_likes WHERE film_id NOT IN ("
